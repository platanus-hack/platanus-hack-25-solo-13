<script lang="ts">
  ...
  let { onQuestClick, onProgressClick, onActivityClick, onMissionsClick, onEventsClick, onLogoutClick, subjectCount, activityCount = 0, missionCount = 0, eventCount = 0 }: Props = $props();
</script>

<nav class="sticky top-[73px] z-40 border-b border-white/5 bg-canvas-950/80 backdrop-blur-md sub-nav-bar">
  <div class="px-6 py-2 flex items-center justify-end gap-2">
    <!-- Quest Button -->
    <button
    ...
    >
      <!-- Map Icon SVG -->
      <svg
        class="w-6 h-6 text-slate-400 group-hover:text-slate-200 transition-colors"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
    ...
    >
      <!-- Logout Icon SVG -->
      <svg
        class="w-6 h-6 text-slate-400 group-hover:text-red-400 transition-colors"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
        xmlns="http://www.w3.org/2000/svg"
